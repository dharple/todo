...
if (count($_POST)) {
	...
	if($_POST['submitButton'] != '') {
		...
		if($_POST['submitButton'] == 'Add') {
		...
		else if($_POST['submitButton'] == 'Rename') {
		...
		else if($_POST['submitButton'] == 'Activate') {
			...
			if($id == "all") {
				...
				if($id > 0) {
					$sections = array(new Section($db, $id));
				}
				else {
					$sections = array(); // skip the next loop
				}
			}
...
// print("Fixing times<br>\n");
		...
		else if($_POST['submitButton'] == 'Deactivate') {
			$id = $_POST['toggle_section_id'];
			if($id == "all") {
				$sectionList = new SimpleList($db, "Section");
